...
class OutlookSearchSchema(BaseModel):
    """邮件搜索参数"""
...
class OutlookReadSchema(BaseModel):
    """邮件阅读参数"""
    index: int = Field(
...
class OutlookSendSchema(BaseModel):
    """邮件发送参数"""
...
class PygamePlaySchema(BaseModel):
    """音乐播放参数"""
    song_name: str = Field(description="歌曲名称")


class PygameControlSchema(BaseModel):
    """音乐控制参数"""
    action: str = Field(
...
class PygameSearchSchema(BaseModel):
    """音乐搜索参数"""
